...
const Campaigns = () => {
  ...
  const [form, setForm] = useState<Partial<TablesInsert<"campaigns">>>({
    campaign_name: "",
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from("campaigns").insert({
      ...
      user_id: user.id,
      status: "draft",
    } as TablesInsert<"campaigns">);
    if (error) { toast({ title: "שגיאה", description: error.message, variant: "destructive" }); return; }
    toast({ title: "קמפיין נוצר בהצלחה" });
    setDialogOpen(false);
    fetchCampaigns();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <Button onClick={() => setDialogOpen(true)} className="gap-2"><Plus className="w-4 h-4" /> קמפיין חדש</Button>
      </div>
            ...
            <Card key={c.id} className="hover:shadow-md transition-shadow">
              ...
              <CardContent>
                <div className="flex gap-2 mb-3">
                  ...
                  <Badge variant={c.status === "sent_simulated" ? "secondary" : "default"}>
                ...
                <div className="flex gap-2">
                  {c.status === "draft" && (
                    <Button size="sm" variant="outline" className="gap-1" onClick={() => updateStatus(c.id, "ready")}>
                      <FileCheck className="w-3 h-3" /> סמן כמוכן
                    </Button>
                  )}
                  {c.status === "ready" && (
                    <Button size="sm" variant="outline" className="gap-1" onClick={() => updateStatus(c.id, "sent_simulated")}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>נושא</Label>
              <Input value={form.subject_template || ""} onChange={(e) => setForm(p => ({ ...p, subject_template: e.target.value }))} />
            </div>
            <div className="space-y-2">
              <Label>תוכן *</Label>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleSave}>צור קמפיין</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
